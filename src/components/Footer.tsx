  ...
  return (
    <footer className="border-t border-white/10 px-5 py-12 sm:px-8">
      <div className="mx-auto flex max-w-6xl flex-col items-center gap-8 text-center sm:flex-row sm:items-start sm:justify-between sm:text-left">
        <div>
          <div className="flex items-center gap-2 text-xl font-black text-white normal-case">
        ...
        <div>
          <p className="text-xs font-bold tracking-wide text-white/40 uppercase">{tr('footer_quick_links')}</p>
          <nav className="mt-3 flex flex-col gap-2">
            {NAV_LINKS.map((link) => (
              <Link key={link.key} to={`/${link.hash}`} className="text-sm text-white/60 transition-colors hover:text-gold">
                {tr(link.key)}
              </Link>
            ))}
          </nav>
        ...
        <div className="flex flex-col items-center gap-3 sm:items-end">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex items-center gap-2 text-sm font-semibold text-white/70 transition-colors hover:text-gold"
          >
